...
const ModernShared = {
  configMenu: new Spicetify.Menu.SubMenu("Modern", []),
  rightBigCover: localStorage.getItem("dribs-right-big-cover") === "true",
  setRightBigCover: () => {
    if (ModernShared.rightBigCover) {
...
ModernShared.configMenu.register();
ModernShared.configMenu.addItem(new Spicetify.Menu.Item(
  "Right expanded cover",
  ModernShared.rightBigCover,
  (self) => {
    self.isEnabled = !self.isEnabled;
    ModernShared.rightBigCover = self.isEnabled;
    localStorage.setItem("dribs-right-big-cover", self.isEnabled);
    ModernShared.setRightBigCover();
  }
));
ModernShared.setRightBigCover();
...
waitForElement([
  ...
  function loadPlaylistImage() {
    for (const item of listElem.children) {
      ...
      if (app === "playlist") {
        ...
        const base64 = localStorage.getItem("Modern:folder-image:" + uid);
  ...
  ModernShared.loadPlaylistImage = loadPlaylistImage;
  loadPlaylistImage();
...
(function Modern() {
    ...
    reader.onload = (event) => {
      ...
      try {
        localStorage.setItem(
          "Modern:folder-image:" + id,
          result
        );
      } catch {
        Spicetify.showNotification("File too large");
      }
      ModernShared.loadPlaylistImage?.call();
    }
  ...
  new Spicetify.ContextMenu.Item("Remove folder image",
    ([uri]) => {
      ...
      localStorage.removeItem("Modern:folder-image:" + id);
      ModernShared.loadPlaylistImage?.call();
    },
